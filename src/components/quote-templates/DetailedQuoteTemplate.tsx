...
export function DetailedQuoteTemplate({ quote, client, settings }: { quote: Quote, client: Client, settings: Settings }) {
  ...
  const emptyRowsCount = quote.items.length <= 18 ? Math.max(0, 18 - quote.items.length) : 0;

  return (
    // A4 sizing and margins simulation with more compact layout
    <div className="bg-white p-4 font-serif text-[9pt] text-gray-800 min-h-[29.7cm] flex flex-col printable-area" id="quote-content">
      {/* Header */}

...
export function DetailedQuoteTemplate({ quote, client, settings }: { quote: Quote, client: Client, settings: Settings }) {
  ...
  return (
    <div id="quote-content" className="printable-area bg-white text-gray-800 font-serif" style={{ fontFamily: '"Times New Roman", Times, serif' }}>
          ...
          <div key={pageIndex} className="p-4 flex flex-col min-h-[29.7cm]" style={{ fontSize: '14pt', pageBreakAfter: isLastPage ? 'auto' : 'always' }}>
            ...
            <header className="flex justify-between items-start mb-4">
              ...
              <div className="w-1/2 text-right">
                <h1 className="text-xl font-bold text-gray-900" style={{ fontSize: '18pt' }}>FACTURE PROFORMA</h1>
                <p className="mt-1">{quote.quoteNumber}</p>
                <p className="mt-0.5 text-gray-500">Date: {format(new Date(quote.date), 'd MMM yyyy', { locale: fr })}</p>
                <p className="text-gray-500">Valable jusqu'au: {format(new Date(quote.expiryDate), 'd MMM yyyy', { locale: fr })}</p>
              </div>
            </header>

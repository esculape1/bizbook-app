    ...
    <div id="invoice-content" className="printable-area bg-white p-8 font-sans text-sm text-black ring-1 ring-gray-200 w-full max-w-[210mm] mx-auto min-h-[297mm] flex flex-col">
            ...
            <table className="w-full text-left border-collapse">
                <thead>
                    <tr className="bg-gray-200 text-[10px] uppercase">
                        <th className="p-2 border border-black w-[45%]">Désignation</th>
                        <th className="p-2 border border-black text-right">Prix Unitaire</th>
                        <th className="p-2 border border-black text-center">Qté</th>
                        <th className="p-2 border border-black text-right">Montant</th>
                    </tr>
                </thead>
                <tbody className="text-xs">
                    {invoice.items.map((item, index) => (
                        <tr key={index}>
                            ...
                            <td className="p-2 border border-black text-right">{formatCurrency(item.unitPrice, settings.currency)}</td>
                            <td className="p-2 border border-black text-center">{item.quantity}</td>
                            <td className="p-2 border border-black text-right font-bold">{formatCurrency(item.total, settings.currency)}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
        ...
        <div className="mt-8 flex justify-end">
            <table className="w-full max-w-[350px] text-xs">
                <tbody>
                    <tr>
                        <td className="p-2 border border-black font-bold uppercase">Sous-Total Brut</td>
                        <td className="p-2 border border-black text-right">{formatCurrency(invoice.subTotal, settings.currency)}</td>
                    </tr>
                    {discount > 0 && (
                        <tr>
                            <td className="p-2 border border-black">Remise ({discount}%)</td>
                            <td className="p-2 border border-black text-right">-{formatCurrency(invoice.discountAmount || 0, settings.currency)}</td>
                        </tr>
                    )}
                    <tr>
                        <td className="p-2 border border-black font-bold uppercase">Sous-Total Hors Taxes</td>
                        <td className="p-2 border border-black text-right font-bold">{formatCurrency(invoice.subTotal - (invoice.discountAmount || 0), settings.currency)}</td>
                    </tr>
                    <tr>
                        <td className="p-2 border border-black">TVA ({vat}%)</td>
                        <td className="p-2 border border-black text-right">+{formatCurrency(invoice.vatAmount || 0, settings.currency)}</td>
                    </tr>
                    <tr>
                        <td className="p-2 border border-black font-bold uppercase">Total TTC</td>
                        <td className="p-2 border border-black text-right font-bold">{formatCurrency(invoice.totalAmount, settings.currency)}</td>
                    </tr>
                    <tr>
                        <td className="p-2 border border-black text-destructive">Retenue à la source ({retenue}%)</td>
                        <td className="p-2 border border-black text-right text-destructive">-{formatCurrency(invoice.retenueAmount || 0, settings.currency)}</td>
                    </tr>
                    <tr className="bg-gray-100 font-black text-sm">
                        <td className="p-2 border border-black uppercase">Net à Payer</td>
                        <td className="p-2 border border-black text-right">{formatCurrency(netAPayer, settings.currency)}</td>
                    </tr>
                </tbody>
            </table>
        </div>

        <footer className="mt-12 pt-4 border-t border-black flex flex-col gap-8">

    ...
    <div id="invoice-content" className="printable-area bg-white p-8 font-sans text-sm text-black ring-1 ring-gray-200 w-full max-w-[210mm] mx-auto min-h-[297mm] flex flex-col">
        <header className="flex justify-between items-start mb-8 pb-4 border-b">
            ...
            <div className="text-right">
                <h1 className="text-2xl font-bold uppercase">Facture</h1>
        ...
        <div className="mb-8 p-4 bg-gray-100 rounded-md">
            <h3 className="font-bold text-gray-600 uppercase text-xs mb-2">Facturé à :</h3>
        ...
        <div className="mt-8 flex justify-end">
            <table className="w-full max-w-[300px]">
                <tbody>
                    <tr><td className="p-2 border font-bold">SOUS-TOTAL</td><td className="p-2 border text-right">{formatCurrency(invoice.subTotal, settings.currency)}</td></tr>
                    {invoice.discount > 0 && <tr><td className="p-2 border">Remise {invoice.discount}%</td><td className="p-2 border text-right">-{formatCurrency(invoice.discountAmount, settings.currency)}</td></tr>}
                    {invoice.vat > 0 && <tr><td className="p-2 border">TVA {invoice.vat}%</td><td className="p-2 border text-right">+{formatCurrency(invoice.vatAmount, settings.currency)}</td></tr>}
                    <tr className="bg-gray-100"><td className="p-2 border font-bold">TOTAL TTC</td><td className="p-2 border text-right font-bold text-lg">{formatCurrency(invoice.totalAmount, settings.currency)}</td></tr>
                    {invoice.retenue > 0 && <tr><td className="p-2 border text-destructive">Retenue {invoice.retenue}%</td><td className="p-2 border text-right text-destructive">-{formatCurrency(invoice.retenueAmount || 0, settings.currency)}</td></tr>}
                    <tr className="bg-black text-white"><td className="p-2 border font-bold uppercase">Net à Payer</td><td className="p-2 border text-right font-bold text-lg">{formatCurrency(invoice.netAPayer || invoice.totalAmount, settings.currency)}</td></tr>
                </tbody>
            </table>
        </div>
        ...
        <footer className="mt-12 pt-4 border-t flex justify-between">
            <div className="text-xs italic text-gray-500">
                ...
                <p className="font-bold uppercase text-black">{numberToWordsFr(invoice.netAPayer || invoice.totalAmount, settings.currency)}</p>
            </div>

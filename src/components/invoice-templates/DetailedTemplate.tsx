    ...
    <>
      <style>{`
        ...
        @media print {
          ...
          .invoice-wrapper { width: 210mm; height: 297mm; box-shadow: none; margin: 0; }
          ...
          .net-a-payer-row { -webkit-print-color-adjust: exact; print-color-adjust: exact; background-color: #111 !important; color: white !important; }
        }
      `}</style>
        ...
        <div className="content-inner">
          <header className="flex justify-between items-start mb-6">
            ...
            <div className="text-right font-bold text-[8pt] md:text-[9pt] shrink-0">
              <p className="opacity-60 uppercase mb-1">Informations</p>
          ...
          <div className="mt-6">
            <table className="totals-table">
              <tbody>
                <tr>
                  ...
                  <td className="text-right font-bold tabular-nums">{formatCurrency(invoice.subTotal, settings.currency)}</td>
                </tr>
                {invoice.discount > 0 && (
                  <tr className="text-gray-600">
                    <td className="uppercase">REMISE {invoice.discount}%:</td>
                    <td className="text-right tabular-nums">-{formatCurrency(invoice.discountAmount, settings.currency)}</td>
                  </tr>
                )}
                {invoice.vat > 0 && (
                  <tr className="text-gray-600">
                    <td className="uppercase">TVA {invoice.vat}%:</td>
                    <td className="text-right tabular-nums">+{formatCurrency(invoice.vatAmount, settings.currency)}</td>
                  </tr>
                )}
                <tr className="font-bold text-[#002060]">
                  <td className="uppercase">TOTAL TTC:</td>
                  <td className="text-right tabular-nums">{formatCurrency(invoice.totalAmount, settings.currency)}</td>
                </tr>
                {retenue > 0 && (
            ...
            <div className="mt-6 border-t border-gray-200 pt-4">
              <p className="font-bold text-[8pt] md:text-[9pt] mb-1">Arrêtée la présente facture à la somme de :</p>
          ...
          <footer className="text-center text-gray-500 text-[7pt] md:text-[7.5pt] border-t border-[#002060] pt-2 mt-auto">
